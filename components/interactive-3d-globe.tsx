...
export function Interactive3DGlobe({ className = '', onNodeHover }: Interactive3DGlobeProps) {
  ...
  useEffect(() => {
    ...
    */
    ...
    // JJSOFT Official Brand Anodized Crimson Hardware Materials
    // Ultra-premium ruby metallic finish with subtle inner warmth and clearcoat specular highlights
    const brandCrimsonSkinMat = new THREE.MeshPhysicalMaterial({
      color: 0xc41429,
      metalness: 0.88,
      roughness: 0.22,
      emissive: 0x220306,
    ...
    const brandCrimsonTrimMat = new THREE.MeshPhysicalMaterial({
      color: 0xe81936,
      metalness: 0.94,
      roughness: 0.12,
      emissive: 0x300408,
    ...
    const brandDarkBezelMat = new THREE.MeshStandardMaterial({
      color: 0x080c14,
      metalness: 0.25,
      roughness: 0.60,
    })

    const brandKeyboardMat = new THREE.MeshStandardMaterial({
      color: 0x111620,
      metalness: 0.35,
      roughness: 0.50,
    })

    const brandTrackpadMat = new THREE.MeshPhysicalMaterial({
      color: 0x221218,
      metalness: 0.70,
      roughness: 0.25,
      clearcoat: 0.45,
    ...
    // B. LAPTOP WORKSTATION (Left Foreground - Premium GLTF Model in Brand Crimson Edition)
    ...
    gltfLoader.load('/assets/models/laptop-premium.glb', (gltf) => {
      ...
      model.traverse((child) => {
        if ((child as THREE.Mesh).isMesh) {
          ...
          } else if (mesh.name === 'bezel') {
            mesh.material = brandDarkBezelMat
          } else if (mesh.name === 'hinges') {
            mesh.material = brandCrimsonTrimMat
          } else if (mesh.name === 'trackpad') {
            mesh.material = brandTrackpadMat
          } else if (mesh.name === 'keyboard') {
            mesh.material = brandKeyboardMat
          } else {
            // base chassis, lid shell, keyboard deck
            mesh.material = brandCrimsonSkinMat
          }
        }
      })
    ...
    // C. SMARTPHONE DEVICE (Center Foreground - Premium GLTF Model in Brand Crimson Edition)
    ...
    gltfLoader.load('/assets/models/phone-premium.glb', (gltf) => {
      ...
      model.traverse((child) => {
        if ((child as THREE.Mesh).isMesh) {
          ...
          } else if (mesh.name === 'buttonL0' || mesh.name === 'buttonL1' || mesh.name === 'back_camera') {
            mesh.material = brandCrimsonTrimMat
          } else {
            mesh.material = brandCrimsonSkinMat
          }
    ...
    // D. TABLET DEVICE WITH JJSOFT GLOBAL LOGO (Right Foreground - Premium GLTF Model in Brand Crimson Edition)
    ...
    gltfLoader.load('/assets/models/tablet-premium.glb', (gltf) => {
      ...
      model.traverse((child) => {
        if ((child as THREE.Mesh).isMesh) {
          ...
          } else if (mesh.name === 'front_camera') {
            mesh.material = new THREE.MeshBasicMaterial({ color: 0x080c14 })
          } else if (mesh.name === 'side_button') {
            mesh.material = brandCrimsonTrimMat
          } else {
            mesh.material = brandCrimsonSkinMat
          }
